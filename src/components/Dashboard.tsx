...
import { ShoppingCart, Zap, Fuel, Globe, Plane, Coffee } from 'lucide-react'; // Example icons
...
const SCENARIOS = [
    ...
    { id: 'overseas', label: '海外消費', icon: Globe },
];
...
export function Dashboard({ userCards }: { userCards: UserCard[] }) {
    ...
    const handleRecord = async (r: RecommendationResult) => {
        ...
        try {
            ...
            alert("記錄成功！");
            // Trigger re-calc (Dependencies will handle if we depended on DB signals, but here we depend on props)
            // Actually we heavily rely on DB state in `calculateRecommendation` but that is not reactive here unless we force update.
            // Force reload or just let it be?
            // For V1, let's trigger a re-mount or refetch. 
            // Ideally `calculateRecommendation` should use live query? No, too complex.
            // We can just simple re-trigger the effect by toggling a signal.
            setScenario(s => s); // Hack: setting same state doesn't trigger effect usually.
            // Let's toggle a counter
            window.location.reload(); // Simplest for V1 to ensure cap updates visually
        } catch (e) {
    ...
    return (
        <div className="bg-gray-100 min-h-screen pb-20">
            ...
            <div className="bg-white p-4 shadow-sm sticky top-0 z-10">
                <h1 className="text-lg font-bold text-center mb-4">最佳刷卡推薦</h1>
                ...
                <div className="relative mb-4">
                    ...
                    <input
                        ...
                        className="w-full pl-8 pr-4 py-3 rounded-xl bg-gray-100 font-mono text-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="輸入金額"
                    />
                </div>
                    ...
                    {SCENARIOS.map(s => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex flex-col items-center gap-1 min-w-[70px] p-2 rounded-lg transition-colors
                ...
                {results.map((r, idx) => (
                    <div key={r.userCard.id} className="bg-white p-4 rounded-xl shadow-sm relative overflow-hidden">
                        ...
                        <div className="bg-gray-50 rounded-lg p-3 flex justify-between items-center mb-3">
                            ...
                            {r.cardDef.rules.find(rule => rule.capType !== 'none') && (
                                <div className="text-right">
                                    <span className="text-gray-500 text-sm">剩餘額度</span>
                                    <div className={`font-bold ${r.remainingCap < 100 ? 'text-red-500' : 'text-gray-800'}`}>
                                        {r.remainingCap === Infinity ? '∞' : `$${r.remainingCap.toFixed(0)}`}
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        {r.warningMessage && (
                            <div className="mb-3 text-xs text-red-500 bg-red-50 px-2 py-1 rounded">
                        ...
                        <button
                            ...
                            className="w-full py-2.5 rounded-lg bg-gray-900 text-white font-medium active:bg-gray-700"
                        >
                            記帳並累積
                        </button>
                    </div>
